...
public class CameraFollower : MonoBehaviour
{
    ...
    Vector3 ClampPosition(Vector3 positionTarget)
    {
        ...
        if (clampToCollider)
        {
            clampedPosition = new Vector3(
                Mathf.Clamp(positionTarget.x, clampToCollider.bounds.min.x + cameraSize.x, clampToCollider.bounds.max.x - cameraSize.x),
                Mathf.Clamp(positionTarget.y, clampToCollider.bounds.min.y - cameraSize.y, clampToCollider.bounds.max.y + cameraSize.y),
                positionTarget.z
            );
        }
    ...
    Vector3 GeneratePositionOffset(Vector3 position) {
        ...
        float x = ((int)axisToBloq != 1)? (followTo.position.x + offsetX * followTo.transform.localScale.x - position.x) * followIntensity : 0;
